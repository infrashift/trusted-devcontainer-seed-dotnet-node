# trusted-devcontainer-seed-dotnet-node
...
## What is in it
    ...
    .devcontainer/          the environment: the InfraShift trusted dotnet-node
                            template (.NET SDK, Node.js, npm and pnpm), plus the workspace runtime
...
`dotnet restore` reads the NuGet group, `npm` and `pnpm` the npm group, each
with your own credential. See `RDW-DEV-GUIDE-DOTNET-NODE.md`.
